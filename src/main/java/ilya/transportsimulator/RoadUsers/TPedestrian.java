package ilya.transportsimulator.RoadUsers;

import ilya.transportsimulator.Geometry.Point2D;
import ilya.transportsimulator.Geometry.Vector2D;
import ilya.transportsimulator.RoadEquipment.PedestrianTrafficLight;
import ilya.transportsimulator.RoadEquipment.StopLine;
import ilya.transportsimulator.RoadEquipment.TrafficLight;
import ilya.transportsimulator.RoadEquipment.TrafficLightState;
import ilya.transportsimulator.Simulation.TrafficSimulationContext;
import ilya.transportsimulator.Math.Math;

import java.util.List;
import java.util.Random;

public final class TPedestrian extends TRoadUser {
    private static final double START_ACCELERATION_COEFFICIENT = 0.5;
    private static final double STOP_ACCELERATION_COEFFICIENT = 0.5;

    private boolean hasCrossedCrosswalk;

    public TPedestrian() {
        super();
        this.hasCrossedCrosswalk = false;
    }

    public TPedestrian(final double maxSpeed, final double acceleration, final double speed, final double minFreeSpaceAround,
    ...
    public void update(final double dt, final TrafficSimulationContext ctx) {
        ...
        for (final StopLine stopLine : ctx.stopLines()) {
            if (!(stopLine.getLinkedTrafficLight() instanceof PedestrianTrafficLight))
                continue;
        ...
        if (closestStopLine != null) {
            if (closestStopLine.getLinkedTrafficLight().getCurrState() != TrafficLightState.GREEN && !this.hasCrossedCrosswalk) {
                ...
                } else if (Math.compareDouble(minDistanceToStopLine, TrafficLight.MIN_FREE_SPACE_AROUND * 3.0 * this.MAX_SPEED / 60) <= 0) {
                    this.stoppingReason = StoppingReason.STOP_LINE;
                    this.acceleration = -1.2 * STOP_ACCELERATION_COEFFICIENT * this.MAX_SPEED;
                }

            } else if (closestStopLine.getLinkedTrafficLight().getCurrState() == TrafficLightState.GREEN && !this.hasCrossedCrosswalk) {
                if (Math.compareDouble(minDistanceToStopLine, this.MIN_FREE_SPACE_AROUND) <= 0)
                    this.hasCrossedCrosswalk = true;
        ...
        if (!shouldStop) {
            ...
            for (final TRoadUser roadUser : ctx.roadUsers()) {
                if (roadUser instanceof TPedestrian)
                    continue;
            ...
            if (closestRoadUser != null) {
                ...
                } else if (Math.compareDouble(minDistanceToRoadUser, this.MIN_FREE_SPACE_AROUND * 2.0 * this.MAX_SPEED / 60) <= 0) {
                    this.stoppingReason = StoppingReason.ROAD_USER;
                    this.acceleration = -1.5 * STOP_ACCELERATION_COEFFICIENT * this.MAX_SPEED;
                }
    ...
    public boolean setupAndAddIntoContext(final TrafficSimulationContext ctx) {
        this.MAX_SPEED = new Random().nextInt(25, 80);
        this.acceleration = 0.0;
        this.speed = this.MAX_SPEED;
        this.MIN_FREE_SPACE_AROUND = 20.0;


        final List<List<Point2D>> potentialPathsForPedestrian = ctx.potentialPathsForRoadUsers().get(TPedestrian.class);
        if (potentialPathsForPedestrian == null || potentialPathsForPedestrian.isEmpty())
            return false;

        final Random rnd = new Random();
        int randomPathIndex;
        List<Point2D> randomPath = null;
        boolean canTPedestrianSpawnCorrectly = false;
        ...
        for (int i = 0; i < 5; ++i) {
            randomPathIndex = rnd.nextInt(potentialPathsForPedestrian.size());
            randomPath = potentialPathsForPedestrian.get(randomPathIndex);
            ...
            final boolean canSpawnPedestrianHere = ctx.roadUsers().stream()
            ...
            if (canSpawnPedestrianHere) {
                canTPedestrianSpawnCorrectly = true;
                break;
            }
        }

        if (!canTPedestrianSpawnCorrectly)
            return false;
    ...
    public boolean hasCrossedCrosswalk() {
        return this.hasCrossedCrosswalk;
    }

    public void setHasCrossedCrosswalk(final boolean hasCrossedCrosswalk) {
        this.hasCrossedCrosswalk = hasCrossedCrosswalk;
    }
    ...
    public String toString() {
        return String.format("TPedestrian[super=%s]", super.toString());
    }
    ...
    public static final class Builder {
        ...
        public TPedestrian build() {
            return new TPedestrian(this.MAX_SPEED, this.acceleration, this.speed, this.MIN_FREE_SPACE_AROUND, this.direction, this.pos, this.brain);
        }
    }
}
