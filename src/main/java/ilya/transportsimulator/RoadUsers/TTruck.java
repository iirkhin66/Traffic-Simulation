...
import ilya.transportsimulator.RoadEquipment.RoadTrafficLight;
...
public final class TTruck extends TRoadUser {
    private static final double START_ACCELERATION_COEFFICIENT = 0.6;
    ...
    public TTruck(final double maxSpeed, final double acceleration, final double speed, final double minFreeSpaceAround,
    ...
    public void update(final double dt, final TrafficSimulationContext ctx) {
        ...
        // 4. если расстояние до него меньше минимального свободного (либо своего, либо его, если он машина), то остановиться
        ...
        if (closestStopLine != null) {
            ...
            if (!(roadTrafficLight.getCurrState() == TrafficLightState.GREEN ||
                ...
                } else if (Math.compareDouble(minDistanceToStopLine, TrafficLight.MIN_FREE_SPACE_AROUND * 4.0 * this.MAX_SPEED / 150) <= 0) {
                    this.stoppingReason = StoppingReason.STOP_LINE;
        ...
        if (!shouldStop) {
            ...
            if (closestRoadUser != null) {
                if (Math.compareDouble(minDistanceToRoadUser, this.MIN_FREE_SPACE_AROUND) <= 0 ||
                        Math.compareDouble(minDistanceToRoadUser, closestRoadUser.MIN_FREE_SPACE_AROUND) <= 0) {
                    this.stoppingReason = StoppingReason.ROAD_USER;
                    shouldStop = true;

                } else if (Math.compareDouble(minDistanceToRoadUser, this.MIN_FREE_SPACE_AROUND * 1.5 * this.MAX_SPEED / 150) <= 0 &&
                        Math.compareDouble(minDistanceToRoadUser, this.MIN_FREE_SPACE_AROUND) > 0) {
                    this.stoppingReason = StoppingReason.ROAD_USER;
    ...
    public boolean setupAndAddIntoContext(final TrafficSimulationContext ctx) {
        this.MAX_SPEED = new Random().nextInt(100, 200);
        this.acceleration = 0.0;
        this.speed = this.MAX_SPEED;
        this.MIN_FREE_SPACE_AROUND = 60.0;

        final List<List<Point2D>> potentialPathsForTrucks = ctx.potentialPathsForRoadUsers().get(TTruck.class);
        if (potentialPathsForTrucks == null || potentialPathsForTrucks.isEmpty())
            return false;

        boolean canTTruckSpawnCorrectly = false;
        ...
        for (int i = 0; i < 5; ++i) {
            randomPathIndex = rnd.nextInt(potentialPathsForTrucks.size());
            randomPath = potentialPathsForTrucks.get(randomPathIndex);
            ...
            final boolean canSpawnTruckHere = ctx.roadUsers().stream()
            ...
            if (canSpawnTruckHere) {
                canTTruckSpawnCorrectly = true;
                break;
            }
        }

        if (!canTTruckSpawnCorrectly)
            return false;
    ...
    public String toString() {
        return String.format("TTruck[super=%s]", super.toString());
    }
    ...
    public static final class Builder {
        ...
        public TTruck build() {
            return new TTruck(this.MAX_SPEED, this.acceleration, this.speed, this.MIN_FREE_SPACE_AROUND, this.direction, this.pos, this.brain);
        }
    }
}
